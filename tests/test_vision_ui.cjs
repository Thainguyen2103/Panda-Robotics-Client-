...
assert.equal(elements.get('cv-emotion').textContent,'Vui (60%), Trung tính (30%), Ngạc nhiên (10%)');
send({emotion_probs:{happy:.30,neutral:.25,surprised:.15,sad:.10,angry:.08,disgust:.06,fear:.04,contempt:.02}},7250);
assert.equal(elements.get('cv-emotion').textContent,'Vui (30%), Trung tính (25%), Ngạc nhiên (15%), Buồn (10%), Giận (8%)','only the top five emotions must be shown');
now=11500;panel.tick();
